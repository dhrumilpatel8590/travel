...
export default function BaliPackages() {
  ...
  const packages = {
    "6n7d": {
      ...
      stays: [
        { area: "4N in Kuta", hotel: "Anathera Resort Kuta (Deluxe Room Non Balcony) or similar" },
        { area: "2N in Jimbaran", hotel: "Bali Paragon Resort Hotel (Deluxe Room) or similar" }
      ],
      itinerary: [
        { day: 1, desc: "Arrival At Bali + Garland Transfer on PVT Basis" },
        { day: 2, desc: "Full Day Water Sports Activity (Jet Ski, Parasailing and Banana Boat) followed by Uluwatu temple tour" },
        { day: 3, desc: "Day free for leisure" },
        { day: 4, desc: "Full Day Bedugul (Ulundanu temple) followed by Tanah Lot Temple (Sunset temple tour)" },
        { day: 5, desc: "Full Day Ubud Tour along with My Swing Coffee Plantation, ubud palace and ubud art market followed by Inter Hotel Transfer on PVT Basis" },
    ...
    "7n8d": {
      ...
      price: "62,500",
      badge: "June Special Deal",
      dates: ["9 June", "12 June", "17 June", "18 June", "21 June", "25 June", "29 June", "30 June"],
      stays: [
        { area: "4N in Kuta", hotel: "Anathera Resort Kuta (Deluxe Room Non Balcony) or similar" },
        { area: "3N in Jimbaran", hotel: "Bali Paragon Resort Hotel (Deluxe Room) or similar" }
      ],
      itinerary: [
        { day: 1, desc: "Arrival At Bali + Garland Transfer on PVT Basis" },
        { day: 2, desc: "Full Day Water Sports Activity (Jet Ski, Parasailing and Banana Boat) followed by Uluwatu temple tour" },
        ...
        { day: 6, desc: "Full Day Ubud Tour along with My Swing Coffee Plantation, ubud palace and ubud art market followed by Inter Hotel Transfer on PVT Basis" },
    ...
    <div className="bg-white rounded-3xl p-5 sm:p-8 lg:p-12 shadow-xl border border-border grid lg:grid-cols-12 gap-8 lg:gap-12 items-stretch lg:items-center">
      <div className="lg:col-span-7 space-y-6 order-2 lg:order-1">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-border">
          <div className="flex flex-wrap items-center gap-2">
            <span className="bg-orange-100 text-cta font-bold text-xs uppercase tracking-wider px-3 py-1.5 rounded-full font-body flex items-center gap-1">
              <Sparkles className="w-3.5 h-3.5 text-cta" /> June Special Departure
            </span>
        ...
        <p className="text-base sm:text-lg text-muted leading-relaxed font-body">
          Our price magic is back without compromise on quality! Experience the ultimate tropical retreat in Bali.
          Enjoy premium stays in Kuta & Jimbaran, return airfare from Ahmedabad, private transfers, and a complete itinerary.
        </p>
        ...
        <div className="bg-background rounded-2xl p-5 sm:p-6 border border-border space-y-4 font-body transition-all duration-300">
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-bold text-sm text-text uppercase tracking-wider mb-2 text-primary font-body flex items-center gap-1.5">🏨 Hotel Stays (or similar)</h4>
            ...
            <div>
              <h4 className="font-bold text-sm text-text uppercase tracking-wider mb-2 text-primary font-body flex items-center gap-1.5">📅 Departure Dates ({currentPkg.dates.length} Options)</h4>
              <div className="flex flex-wrap gap-1.5 mt-2">
                {currentPkg.dates.map((date) => (
                  <span key={date} className="bg-white border border-border px-2.5 py-1.5 rounded-xl text-xs font-bold shadow-sm text-primary font-body">
                    {date}
                  </span>
                ))}
              </div>
              <p className="text-xs text-muted mt-4">✈️ Return airfare via <strong className="text-text">Vietjet Airline</strong> included (Non-refundable).</p>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs text-muted font-body list-none pl-0 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar">
              {currentPkg.itinerary.map((step) => (
                <li key={step.day}>
                  <strong className="text-text font-bold">Day {step.day}:</strong> {step.desc}
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="pt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-t border-border font-body">
          <div>
            ...
            <p className="text-[10px] text-muted mt-0.5 font-bold">Powered by DMC Hub | BALItrip</p>
          </div>
      ...
      <div className="lg:col-span-5 relative aspect-[4/3] lg:aspect-auto w-full min-h-[250px] sm:min-h-[350px] lg:min-h-[550px] lg:h-full order-1 lg:order-2">
        ...
        <div className="absolute bottom-4 left-4 bg-cta/95 backdrop-blur-md text-white px-4 py-3 rounded-xl shadow-lg font-body text-xs flex flex-col gap-0.5">
          <span className="font-bold uppercase tracking-wider text-[10px]">Flight Included</span>
